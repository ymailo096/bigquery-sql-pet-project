...
Серпень 2024 — загальна виручка **€105,175** по **50 унікальних замовленнях**.

- **MAILO** генерує найбільше виручки (€47,265), **KUBRAK** лідирує за кількістю замовлень (27 vs 20)
- Маршрут **CZ→CZ** — найефективніший за €/km (~4.0), що втричі перевищує середній показник по компанії
- **Чехія** домінує на карті як основна точка призначення (€55,895 — більше половини загальної виручки)
- На scatter plot видно два сині outliers (KUBRAK) з ціною ~€5,000 при відстані 900+ km — аномально висока вартість рейсу відносно дистанції
- Два піки навантаження: **7-9 серпня** і **19-21 серпня**, між якими помітний спад активності
- **DYPTAN** має лише 3 замовлення — найменше навантаження серед менеджерів
> **Примітка:** `Orders Count` на дашборді використовує `COUNT DISTINCT` по `internal_order_id` — round trip рейси мають спільний ID, тому відображається менша кількість. Фактична кількість рядків у таблиці фактів: MAILO — 52, KUBRAK — 37, DYPTAN — 5.
